...
const ACCENT_DARK = {
  white: 'bg-gradient-to-b from-white/5 to-white/0 border-white/10 text-white',
  amber: 'bg-gradient-to-b from-amber-500/10 to-amber-500/0 border-amber-500/20 text-amber-400',
  cyan:  'bg-gradient-to-b from-cyan-500/10 to-cyan-500/0 border-cyan-500/20 text-cyan-400',
};
const ACCENT_LIGHT = {
  white: 'bg-white border-slate-200 text-slate-900',
  amber: 'bg-white border-amber-200 text-amber-600',
  cyan:  'bg-white border-cyan-200 text-cyan-700',
};
...
const StatCard = ({ label, value, accent }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
      className={`rounded-2xl border p-5 shadow-sm ${clrs}`}>
      ...
      <BorderGlow borderRadius={28}>
        <div className="p-6">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h3 className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-widest">Skill Inventory</h3>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              <AnimatePresence>
                {skills.map((skill, idx) => {
                  ...
                  const emptyStarColor = isLight ? '#cbd5e1' : '#ffffff10';
                    ...
                    <motion.div key={skill._id}
                      ...
                      className={`p-5 rounded-2xl border transition-all shadow-sm ${
                        isLight
                          ...
                          : 'bg-white/[0.04] border-white/5 hover:border-white/10 hover:bg-white/[0.07]'
                      }`}>
                        ...
                        <div className="flex items-center gap-1">
                          {[1,2,3,4,5].map(i => (
                            <Star key={i} className="w-3 h-3"
                              style={{ color: i <= lvl ? color : emptyStarColor, fill: i <= lvl ? color : 'transparent' }} />
                          ))}
                        </div>
                      </div>
                      <h4 className="text-sm font-bold text-slate-900 dark:text-white mb-0.5">{skill.skillId?.name || '—'}</h4>
                      ...
                      <div className="flex items-center justify-between mb-1.5">
                        ...
                        <span className="text-[10px] text-slate-500">{skill.yearsOfExperience}yr exp</span>
                      </div>
